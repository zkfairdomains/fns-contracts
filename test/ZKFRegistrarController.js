...
describe("Controller Contract", function () {
    ...
    async function deploy(deployer) {
        ...
        registrar = await ethers.deployContract("FIFSRegistrar", [registry.target, namehash.hash(tld)]); 
        await registrar.waitForDeployment();

        await registry.setSubnodeOwner(ZERO_HASH, labelhash(tld), registrar.target);
        
        reverseRegistrar = await ethers.deployContract("ReverseRegistrar", [registry.target]);
        await reverseRegistrar.waitForDeployment();
 
        await registry.setSubnodeOwner(namehash.hash("reverse"), labelhash("addr"), reverseRegistrar.target);
         
        baseRegistrarImplementation = await ethers.deployContract("BaseRegistrarImplementation",[registry.target, namehash.hash(tld)]);
        ...
        zkfRegistrarController = await ethers.deployContract("ZKFRegistrarController",[baseRegistrarImplementation.target, stablePriceOracle.target, 3, 8400, reverseRegistrar.target, registry.target]);
        ...
        publicResolver = await ethers.deployContract("PublicResolver",[registry.target, zkfRegistrarController.target, reverseRegistrar.target]);
